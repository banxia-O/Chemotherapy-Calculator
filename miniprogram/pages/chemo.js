...
Page({
  ...
  onCancerChange(e) {
    ...
    const cancer = this.data.cancerTypes[index];
    const regimenList = this.regimens[cancer.id] || [];
  ...
  onRegimenChange(e) {
    const index = e.detail.value;
    const regimen = this.data.regimenList[index];
  ...
  // 计算剂量
  calculate() {
    const { height, weight, bsa, selectedRegimen } = this.data;
    ...
    const drugs = selectedRegimen.drugs.map(drug => {
      let calculatedDose = '';
      let standardDisplay = drug.range ? `${drug.range}${drug.unit}` : `${drug.dose}${drug.unit}`;
  ...
  calculateCarboplatin() {
    const { age, weight, gender, creatinine, targetAUC, aucValues } = this.data;
    ...
    const auc = aucValues[targetAUC];
    
    // 肌酐单位转换 μmol/L -> mg/dL
    const creatinineMgDl = creatinineNum / 88.4;
    
    // Cockcroft-Gault公式计算GFR
